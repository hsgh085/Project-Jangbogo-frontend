...
const TCS = () => {
  return (
    <View>
        <Header>
          <Header.Title size={18} style={styles.Header}>서비스 이용약관</Header.Title>
        </Header>
        <ScrollView style={styles.scrollView}>
        <Text style={styles.Text1}>이용자님의 권리 및 의무를 이해하기 위하여 애플리케이션 소프트웨어 이용약관(이하 "본 약관")을 주의 깊게 읽어주시길 바랍니다.</Text>
        <Text>제1조 목적
          본 약관은 장보고(이하 ‘회사’)가 스마트 기기를 통해 제공하는 모든 애플리케이션, 기타 제반 서비스(이하 ‘서비스’)를 이용하는 사용자(이하 ‘이용자’)와 회사 간에 서비스의 이용에 관한 권리, 의무 및 책임 사항, 기타 필요한 사항을 규정함을 목적으로 합니다.\
